...
Shorter output or a cleaner main context does not automatically reduce total cost. Claim savings only from repeated paired runs with independent quality checks and equal verification. Hold host/model/settings fixed except the factor being evaluated; for model comparisons apply [model-selection.md](model-selection.md). Count parent and worker usage, retries, and integration work; distinguish cached usage, monetary cost, and elapsed time where measurable. Missing usage means no savings claim. Keep a cost optimization only when total cost falls without weaker results.
